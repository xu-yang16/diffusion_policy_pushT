...
def replay_simulation():
    ...
    for i in range(len(imgs)):
        ...
        if i < len(reward_data):
            ax1.set_title(f'Environment (Replay) - Frame {i}, Reward: {reward_data[i]:.4f}')
...
fig.text(0.5, 0.95, "Simulation complete. Press 'R' key or click 'Replay' button to replay.", 
         ha='center', va='center', fontsize=12, bbox=dict(boxstyle="round,pad=0.5", facecolor='lightgray'))
...
def create_animation():
    ani = animation.ArtistAnimation(fig, 
                                   [[img_plot.set_data(img)] for img in imgs], 
                                   interval=50, 
                                   blit=True)
    return ani
